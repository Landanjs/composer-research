...
class DeepLabV3Hparams(ModelHparams):
    ...
    backbone_arch: str = hp.optional(
        "The backbone architecture to use. Must be either ['resnet50', resnet101'].",
        default='resnet101')
    is_backbone_pretrained: bool = hp.optional(
        "If true, use pre-trained weights for backbone.", default=True)
    backbone_url: str = hp.optional(
        "Url to download model weights from. If blank (default), will download from PyTorch's url.",
        default='')
    use_plus: bool = hp.optional(
        "If true (default), use DeepLabv3+ head instead of DeepLabv3.",
        default=True)
    ...
    squared_pred: bool = hp.optional("blank", default=False)
    ...
    def validate(self):
        ...
        if self.backbone_arch not in ['resnet50', 'resnet101']:
            raise ValueError(
                f"backbone_arch must be one of ['resnet50', 'resnet101']: not {self.backbone_arch}"
            )
